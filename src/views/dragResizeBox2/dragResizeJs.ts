import { numAdd, numSub, numMulti, isInview, mouseMoveDirection } from '@/utils/index'
...
export class dragResize {
  ...
  wheelZoom() {
    this.wrapperPannel.addEventListener('wheel', (e) => {
      ...
      // 限制缩放倍数
      const _scale = numMulti(scale, ratio);
      if (_scale > maxScale) {
      ...
      const origin = {
        x: (ratio - 1) * numMulti(wrapperWH.width, 0.5),
        y: (ratio - 1) * numMulti(wrapperWH.height, 0.5)
      };
      // 计算偏移量
      x -= numMulti((ratio - 1), (e.clientX - wrapperPannelRect.left - x)) - origin.x;
      y -= numMulti((ratio - 1), (e.clientY - wrapperPannelRect.top - y)) - origin.y;
      // }
      this.changeStyle()
      e.preventDefault();
    });
