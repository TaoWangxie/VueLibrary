<template>
        ...
        >
          ...
          >
            <div v-if="item.comKey !== 'Placeholder'" class="grid_item_header">
              ...
              {{ item.comName }}
              <el-icon
                v-if="useSchema.curGrid && useSchema.curGrid.i === item.i"
                :size="18"
                style="margin-left: 5px"
              >
                <StarFilled />
              </el-icon>
              <span class="grid_info"></span>
            </div>
            <component
              v-if="item.comKey"
              class="component"
  ...
  <CreateFile
    v-model:visible="createFileVisible"
    title="文件预览"
    :width="360"
  ></CreateFile>
</template>
...
import CreateFile from '../../components/CreateFile/index.vue'
...
onBeforeMount(() => {
  ...
  proxy.mittBus.on('createPage', (data: any) => { createPage() })
})
onMounted(async () => {
...
let createFileVisible = ref(false)
const createPage = () => {
