...
import { tem_html, tem_js, tem_css, tem_dialog_html, tem_dialog_js } from '../static/template'
...
export function getCode(page: any) {
    ...
    if (Fdom) {
        ...
        let js = replaceJsCode(page)
        let htmlcode = page.config.type === 'dialog' ? tem_dialog_html(getHtmlStrFn(Rdom, page, replaceHtmlCode)) : tem_html(getHtmlStrFn(Rdom, page, replaceHtmlCode))
        let jscode = page.config.type === 'dialog' ? tem_dialog_js(js.js) : tem_js(js.js)
        let res: any = {
            html: htmlcode,
            js: jscode,
            datajs: js.datajs,
            css: tem_css(getCssStrFn(styleLists))
        }
...
const replaceJsCode = (page: any) => {
    ...
    page.data.map((item: any) => {
        ...
        if (comData) {
            ...
            let str: string = `export const ${dataName + '_' + item.i} = ${JSON.stringify(comData, null, 4)}`
        ...
        if (funcList) {
            ...
            if (['BtnBox', 'Table'].includes(item.comKey)) {
    ...
    let dialogStr: any = `export const dialogs:any = ${JSON.stringify(dialog, null, 4)}`
    importComArr.push(`dialogs`)
    // codeArr.push(dialogStr)
    datajsArr.push(dialogStr)
    codeArr.unshift([
        `const dom:any = shallowReactive({ ${importDialogArr.join(", ")} });`,
        `let visible: any = ref(false);`,
        `let dialog: any = ref(null);`,
        `const methods: any =  {
    weekupDialog: (btn: any) => {
        dialog.value = dialogs[btn.code]
        visible.value = true
    }
}`
    ].join("\n"))
    ...
    //import导入配置
    codeArr.unshift([
        `import {${importComArr.join(", ")} } from "./data.ts";`,
        `const { proxy } = getCurrentInstance() as any;`,
        // `const methods = methodsFn(proxy);`,
    ].join("\n"))
    importDialogArr.map((item) => {
        let importStr = `import ${item} from "./components/${item}/index.vue";`
        codeArr.unshift(importStr)
    })
    return {
        js: codeArr.join("\n"),
        datajs: datajsArr.join("\n")
    }
}
