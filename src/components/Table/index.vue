...
watch(
  () => props.scrollValue,
  newVal => {
    table.value && table.value.setScrollTop(newVal)
  },
  { immediate: true }
)
nextTick(() => {
  scrollTableRef.value = document.querySelector(`.${props.tableClassName} .el-scrollbar__wrap`)
  console.log(scrollTableRef.value);
  scrollTableRef.value?.addEventListener('scroll', () => {

    proxy.$emit('handleScroll', scrollTableRef.value.scrollTop)
  })
})
