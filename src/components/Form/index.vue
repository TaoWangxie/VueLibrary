    ...
    <template v-for="(item, index) in columns">
      ...
      >
        ...
        <el-input
          ...
          v-bind="item"
          :disabled="fields[item.prop].disabled"
        ...
        >
          <el-option
            ...
            :value="item.valueKey ? it : setOptionLabelvalue(it, item, 'value')"
        ...
        <div v-if="item.fieldType === 'upload'" class="upload_box">
          <div v-if="!uploadFile" class="upload_btn" @click="upload(item.prop)">
            上传
          </div>
...
watch(
  ...
  (val) => {
    val.map((item) => {
      fields.value[item.prop] = {
        hide: false,
        disabled: props.whetherDisabled
      }
    })
  },
...
const setFormHide = (hides: any, shows: any, isDisabled?: boolean) => {
  Object.keys(fields.value).map((prop: any) => {
    if (!hides && !shows) {
      fields.value[prop]['hide'] = false
      fields.value[prop]['disabled'] = false
    } else {
      if (shows && shows.includes(prop)) {
        fields.value[prop][isDisabled ? 'disabled' : 'hide'] = false
      }
      if (hides && hides.includes(prop)) {
        fields.value[prop][isDisabled ? 'disabled' : 'hide'] = true
      }
    }
...
const confirmRule = async () => {
  ...
  Object.keys(fields.value).map((item: any) => {
    if (fields.value[item].hide) {
...
const utilsSetData = (curObj: any, targetObj: any, replaceObj?: any) => {
  ...
  Object.keys(curObj).map((item: any) => {
    if (keys.includes(item)) {
      curObj[item] = targetObj[item] || targetObj[item] === 0 || targetObj[item] === false ? targetObj[item] : curObj[item]
    } else if (keys2.length && keys2.includes(item)) {
      let prop = targetObj[replaceObj[item]]
      curObj[item] = targetObj[replaceObj[item]] || targetObj[replaceObj[item]] === 0 || targetObj[replaceObj[item]] === false ? targetObj[replaceObj[item]] : curObj[item]
    }
  })
  return curObj
}
...
//暴露方法给父组件
defineExpose({ confirmRule, resetFields, utilsSetData, setFormHide })
